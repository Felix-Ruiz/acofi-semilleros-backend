...
def obtener_perfil_estudiante(estudiante_id):
    try:
        ...
        return jsonify({
            "id": estudiante.id, "nombre": estudiante.nombres_apellidos, "documento": estudiante.documento_identidad,
            "institucion": estudiante.institucion, "correo": estudiante.correo, "ciudad": estudiante.ciudad,
            "trabajo_titulo": ponencia.titulo if ponencia else estudiante.nombre_trabajo,
            "codigo": ponencia.codigo if ponencia else None, "url_qr": ponencia.url_qr if ponencia else None
        }), 200
    except Exception as e:
...
def toggle_asistencia(id):
    ...
    try:
        ...
        if not estudiante: return jsonify({"error": "Estudiante no encontrado"}), 404
        nuevo_estado = data.get('asistencia', False)
        estudiante.asistencia = nuevo_estado
        db.session.commit()
            ...
            if p and p.url_qr and estudiante.correo:
                ...
                <div style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: auto; border: 1px solid #e5e7eb; border-radius: 12px; padding: 24px;">
                    ...
                    <div style="background-color: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #1e3a8a;">
                        ...
                        <ul style="list-style-type: none; padding-left: 0; margin: 0;">
                            <li style="margin-bottom: 8px;">👤 <strong>Usuario:</strong> Tu documento ({estudiante.documento_identidad})</li>
                ...
                """
                threading.Thread(target=enviar_correo, args=(estudiante.correo, asunto, cuerpo_html)).start()
                mensaje_extra = " y credencial enviada por correo"

        return jsonify({"mensaje": f"Asistencia actualizada con éxito{mensaje_extra}", "asistencia": estudiante.asistencia}), 200
    except Exception as e:
        db.session.rollback()
        return jsonify({"error": str(e)}), 500
...
def obtener_evaluadores():
    try:
        ...
        for e in evaluadores:
            # Incluir asignaciones si existen
            asignadas = [p.codigo for p in e.ponencias_asignadas] if hasattr(e, 'ponencias_asignadas') else []
            resultado.append({
                "id": e.id, "nombres_apellidos": e.nombres_apellidos, "documento_identidad": e.documento_identidad, 
                "institucion": e.institucion, "correo": e.correo, "cargo": e.cargo, "evento_id": e.evento_id, 
                "evento": e.evento.nombre if e.evento else "N/A", "pin_acceso": e.pin_acceso,
...
# ⚠️ NUEVA RUTA PARA ASIGNAR PONENCIAS Y ENVIAR CORREO A EVALUADOR
@admin_bp.route('/evaluadores/<int:id>/asignar', methods=['POST'])
...
# ⚠️ NUEVA RUTA: DASHBOARD EVALUADOR (Para que vean lo que les falta evaluar)
@admin_bp.route('/evaluador_dashboard/<string:documento>', methods=['GET'])
def evaluador_dashboard(documento):
    try:
        evaluador = Evaluador.query.filter_by(documento_identidad=documento).first()
        if not evaluador: return jsonify({"error": "Evaluador no encontrado"}), 404
        
        # Evaluaciones ya realizadas por este evaluador
...
def obtener_ranking():
    try:
        ...
        for p in ponencias:
            ...
            # ⚠️ NUEVO: Listar qué evaluadores han calificado esta ponencia
            nombres_evaluadores = []

            for ev in evaluaciones_db:
                if ev.evaluador: nombres_evaluadores.append(ev.evaluador.nombres_apellidos)
            ...
            resultado.append({
                "id": p.id, "titulo": p.titulo, "codigo": p.codigo or "N/A", "estudiante_nombre": nombres, 
                "num_evaluaciones": num_evals, "puntaje_total": total_score, "promedio": round(promedio, 2),
                "evaluadores_nombres": nombres_evaluadores # ⚠️ Dato extra para el Frontend
            })
...
def eliminar_evaluador(id):
    try:
        ...
        # Eliminar las relaciones de asignaciones antes de borrar
...
def borrar_todos(entidad):
    try:
        if entidad == 'ponencias':
            ...
            # Limpiar tabla de asignaciones para evitar error referencial
    ...
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                if not cargo: cargo = "Participante"

                titulo = get_val(row, col_titulo)
                if not titulo: titulo = "Trabajo General"
        ...
        return jsonify({"mensaje": "Archivo procesado. Se guardaron todos los estudiantes sin omitir."}), 200
    except Exception as e:
        db.session.rollback()
